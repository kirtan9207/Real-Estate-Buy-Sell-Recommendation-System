...
def run_cleaning_pipeline(
    ...
    print(f"✅ Step 2: Duplicates — {rows_before - len(df)} removed (ID: {id_dupes}, Feature: {feature_dupes})")
    ...
    def remove_location_outliers(group):
        ...
        upper = q3 + 1.5 * iqr
        before = len(group)
        filtered = group[(group['price_per_sqft'] >= lower) & (group['price_per_sqft'] <= upper)]
    ...
    df = df.groupby('location', group_keys=False).apply(remove_location_outliers).reset_index(drop=True)
    ...
    print(f"✅ Step 3: Outliers — {total_outliers} removed across {len(outlier_details)} locations")
    ...
    df = df[~((df['property_type'] != 'Plot') & (df['floor'] > df['total_floors']))]
    validations.append(f"Removed {invalid_floor} rows with floor > total_floors")
    ...
    print(f"✅ Step 4: Validation — {rows_before - len(df)} invalid rows removed")
    ...
    if 'price' in numeric_cols:
        ...
        high_corr = correlations[(correlations > 0.95) & (correlations.index != 'price')]
        ...
        print(f"✅ Step 5: Leakage check — {'No leakage detected' if not leakage_cols else f'Flagged: {leakage_cols}'}")
    ...
    print(f"   Removed: {initial_rows - final_rows} rows ({100 - report['summary']['retention_rate']:.2f}%)")
