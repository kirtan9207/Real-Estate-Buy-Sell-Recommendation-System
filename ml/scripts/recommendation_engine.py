...
def run_recommendation_engine(
    ...
    if 'predicted_price' not in df.columns:
        print(" predicted_price column missing. Run train_models.py first.")
    return

    df['price_gap'] = df['price'] - df['predicted_price']
    df['price_gap_pct'] = (
        (df['price_gap'] / df['predicted_price']) * 100).round(2)
    ...
    overpriced = df[df['valuation_label'] == 'Overpriced'].sort_values(
        'price_gap_pct', ascending=False)
    ...
    valuation_stats = {
        ...
        'top_underpriced': underpriced.head(20)[
            ['property_id', 'location', 'price', 'predicted_price',
                'price_gap_pct', 'sqft', 'bedrooms']
        ].to_dict('records'),
    ...
    # SAVE
    # ═══════════════════════════════════════════════════════
    # Update production_final with new columns
