...
def generate_eda_report(
 ...
 ax.hist(df['price'] / 1e7, bins=50, color='#2563eb', alpha=0.7, edgecolor='#1e40af')
 ax.set_xlabel('Price (₹ Cr)')
 ax.set_ylabel('Count')
 ax.set_title('Property Price Distribution — Bangalore', fontweight='bold', fontsize=14)
 plt.tight_layout()
 ...
 ax.scatter(subset['sqft'], subset['price'] / 1e7, alpha=0.3, s=10, label=loc)
 ax.set_xlabel('Square Feet')
 ax.set_ylabel('Price (₹ Cr)')
 ax.set_title('Price vs Square Footage by Location', fontweight='bold', fontsize=14)
 ax.legend(bbox_to_anchor=(1.05, 1), fontsize=7, markerscale=2)
 plt.tight_layout()
 plt.savefig(f'{assets_dir}/price_vs_sqft.png', dpi=150, bbox_inches='tight')
 plt.close()
 ...
 ax.set_ylabel('Price (₹)')
 ax.set_title('Price Distribution by Location', fontweight='bold', fontsize=14)
 plt.tight_layout()
 ...
 df['bedrooms'].value_counts().sort_index().plot(kind='bar', color='#8b5cf6', ax=ax1)
 ax1.set_title('Bedroom Count Distribution', fontweight='bold')
 ax1.set_xlabel('Bedrooms')
 df.groupby('bedrooms')['price'].mean().plot(kind='bar', color='#10b981', ax=ax2)
 ax2.set_title('Average Price by Bedrooms', fontweight='bold')
 ax2.set_xlabel('Bedrooms')
 ax2.set_ylabel('Avg Price (₹)')
 plt.tight_layout()
 plt.savefig(f'{assets_dir}/bedroom_analysis.png', dpi=150, bbox_inches='tight')
 plt.close()
 ...
 ax.plot(age_avg.index, age_avg.values, color='#ef4444', linewidth=2, label='Mean')
 ...
 fig, ax = plt.subplots(figsize=(10, 6))
 ax.scatter(df['distance_metro'], df['price'] / 1e7, alpha=0.2, s=8, c='#2563eb')
 ax.set_xlabel('Distance to Metro (km)')
 ax.set_ylabel('Price (₹ Cr)')
 ax.set_title('Metro Distance vs Price', fontweight='bold', fontsize=14)
 plt.tight_layout()
 plt.savefig(f'{assets_dir}/distance_vs_price.png', dpi=150, bbox_inches='tight')
 plt.close()

 # 8. Market Trend by Location
 fig, ax = plt.subplots(figsize=(12, 6))
 trend_avg = df.groupby('location')['market_trend'].mean().sort_values(ascending=False)
 colors = ['#10b981' if v > 0.12 else '#2563eb' if v > 0.08 else '#f59e0b' for v in trend_avg.values]
 trend_avg.plot(kind='bar', color=colors, ax=ax)
 ax.set_ylabel('Avg Market Trend')
 ax.set_title('Market Growth Trend by Location', fontweight='bold', fontsize=14)
 ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right')
 plt.tight_layout()
 ...
 # 9. ROI Distribution
 fig, ax = plt.subplots(figsize=(10, 6))
 ax.hist(df['roi'], bins=40, color='#10b981', alpha=0.7, edgecolor='#059669')
 ax.axvline(df['roi'].mean(), color='#ef4444', linestyle='--', linewidth=2, label=f"Mean: {df['roi'].mean():.1f}%")
 ax.set_xlabel('ROI (%)')
 ax.set_title('ROI Distribution', fontweight='bold', fontsize=14)
 ax.legend()
 plt.tight_layout()
 plt.savefig(f'{assets_dir}/roi_distribution.png', dpi=150, bbox_inches='tight')
 plt.close()
 ...
 plt.colorbar(scatter, label='Price (₹ Cr)')
 ax.set_xlabel('Demand Score')
 ax.set_ylabel('Liquidity Score')
 ax.set_title('Demand vs Liquidity (colored by Price)', fontweight='bold', fontsize=14)
 plt.tight_layout()
 plt.savefig(f'{assets_dir}/demand_liquidity.png', dpi=150, bbox_inches='tight')
 plt.close()
...
<body>
 ...
 <div class="stat-card"><div class="stat-label">AVG PRICE</div><div class="stat-value">₹{df['price'].mean()/1e7:.2f}Cr</div></div>
