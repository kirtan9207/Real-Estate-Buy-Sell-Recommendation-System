...
def run_clustering(
    ...
    cluster_features = ['price', 'roi', 'location_score', 'luxury_score', 'demand_score']
    ...
    plt.tight_layout()
    plt.savefig(f'{assets_dir}/elbow_silhouette.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    # ═══════════════════════════════════════════════════════
    colors_map = {'Budget': '#6366f1', 'Mid-range': '#2563eb', 'Premium': '#f59e0b', 'Emerging High-Growth': '#10b981'}
    ...
    ax.set_xlabel('Price (₹ Cr)', fontsize=12)
    ax.set_ylabel('ROI (%)', fontsize=12)
    ax.set_title('Market Segmentation — Price vs ROI', fontsize=14, fontweight='bold')
    ax.legend(fontsize=10)
    plt.tight_layout()
    plt.savefig(f'{assets_dir}/cluster_scatter.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    loc_cluster = df.groupby(['location', 'segment_label']).size().unstack(fill_value=0)
    ...
    print(f"\n{'='*50}")
    print(f"🏘️ CLUSTERING SUMMARY")
    print(f"{'='*50}")
    for seg, row in seg_summary.iterrows():
        print(f"   {seg}: {int(row['count'])} properties | Avg ₹{row['avg_price']:,.0f} | ROI {row['avg_roi']:.1f}%")
