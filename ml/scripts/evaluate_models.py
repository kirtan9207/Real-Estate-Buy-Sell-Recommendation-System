...
def evaluate_models(
    ...
    for i, v in enumerate(values):
        axes[idx].text(i, v, f'{v:,.0f}' if metric !=
                       'r2' else f'{v:.4f}', ha='center', va='bottom', fontsize=9)
    plt.suptitle('Model Comparison', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{assets_dir}/model_comparison.png',
    ...
    plt.tight_layout()
    plt.savefig(f'{assets_dir}/residual_vs_predicted.png',
                dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'{assets_dir}/high_error_analysis.png',
                dpi=150, bbox_inches='tight')
    plt.close()
    ...
    for name, r in results.items():
        print(f" {name}: R²={r['r2']} | RMSE=Rs.{r['rmse']:,} | MAPE={r['mape']}%")
